...
# 生成数据源配置
echo -e "${YELLOW}生成Grafana数据源配置...${NC}"
mkdir -p "${GF_DIR}/datasources"
chmod +x "${GF_DIR}/generate-datasource.sh"

# 设置环境变量并生成配置
export VICTORIA_LOGS_HOST
export VICTORIA_LOGS_PORT
bash "${GF_DIR}/generate-datasource.sh"
...
echo -e "\n${GREEN}访问地址:${NC}"
echo -e "📈 Grafana界面: ${BLUE}http://$(hostname -I | awk '{print $1}'):3000${NC}"
...
echo -e "\n${GREEN}数据源配置:${NC}"
echo -e "🎯 VictoriaLogs: ${BLUE}${VICTORIA_LOGS_HOST}:${VICTORIA_LOGS_PORT}${NC}"

echo -e "\n${GREEN}测试命令:${NC}"
echo -e "${BLUE}curl \"http://localhost:3000/api/health\"${NC}"
echo -e "${BLUE}docker logs grafana${NC}"

echo -e "\n${GREEN}停止命令:${NC}"
echo -e "${BLUE}cd ${GF_DIR} && docker-compose down${NC}"
